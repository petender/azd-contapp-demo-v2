...
1. **Azure CLI** version 2.50 or later installed
1. **Azure Developer CLI - AZD** installed (for Option B deployment choice)
...
> **Important**: Choose **ONE deployment option** below. If you are new to Docker and Container Registry tasks such as building, deploying and managing containers, we recommend using **Option A (az cli)**, as it walks through all steps manually. If you are already familiar with Docker, choose **Option B (azd)**, since azd builds and deploys the container images automatically.
...
### Create an Azure DevOps Project (if needed)
